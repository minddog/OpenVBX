...
describe "Messages/{SID}/Replies/SmsMessages" do
  ...
  before(:each) do
    ...
    @resourceUrl = "api/#{@api_version}/Messages/1/Replies/SmsMessages.xml"
  end
  ...
  it "should return a list of replies for a message instance" do
    ...
    resp.should have_nodes("//Annotation[Type!='sms']", 0)
  end

  it "should make a phone call back to the caller" do
    resp = make_request({ 
                          :from => @testing_twilio_number,
                          :to => @testing_phone_number,
                          :body => 'Testing message from functional tester.'
                        }, 'POST')

    resp.should have_nodes("/Response/SmsMessage", 1)
    resp.should have_at_least_num_nodes("/Response/SmsMessage/MessageSid", 1)
